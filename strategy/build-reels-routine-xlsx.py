...
Two additions, both of them additive:
  ...
  2. A new 'Reels & Video Routine' tab, weeks 2 to 24.
...
assert len(header) == len(WIDTHS) == 27 and len(data) == 69
...
h = wb["How to Use"]
assert all(h.cell(r, c).value is None for r in range(18, 32) for c in (2, 3))
...
band_row(29, "GBP DAILY BACKFILL")
pair(30, "Added", "Aug 17 and Aug 18, marked Published. This tab was started on Aug 19, so the first two days of the plan were never logged. It now starts on Aug 17 with everything else.")
pair(31, "No image links", "Left blank on those two rows on purpose. They are a record of posts already made, not something to publish again. Their Notes cell says the copy is reconstructed from that day's Instagram post.")
for r in list(range(20, 28)) + [30, 31]:
